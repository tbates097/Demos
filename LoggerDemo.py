...
root = tk.Tk()
root.title("TextLogger/DecodeFaults Demo")
...
class MockA1Commands:
    def acknowledgeall(self, task):
        print(f"CONTROLLER: AcknowledgeAll command received.")
...
def demo_decode_faults():
    ...
    # Format and write summary to logger widget
    summary = []
    for axis, faults in decoded_results.items():
        if faults:
            summary.append(f"{axis.replace('Axis', 'Axis ')} has {', '.join(faults)}")
        else:
            summary.append(f"{axis.replace('Axis', 'Axis ')} has no faults")
    logger.write("; ".join(summary) + "\n\n")
    # Restore sys.stdout and print the rest to the console
    sys.stdout = sys.__stdout__
...
logger.write("Welcome to the TextLogger/DecodeFaults Demo!\n"
